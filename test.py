...
# ---------------- Config ----------------
checkpoint_path = "checkpoints/ddpm_epoch47.pth"
save_dir = "results/generated_t1"
...
for batch in test_loader:
    ...
    curr_batch_size = t1.shape[0]

    pred = diffusion.sample(x_cond=t1, batch_size=curr_batch_size, cond_scale=cfg_scale)
    pred_01 = denorm01(pred)
    t2_01 = denorm01(t2)

    for i in range(curr_batch_size):
        if "t1_name" in batch:
            in_name = batch["t1_name"][i]
        elif "filename" in batch:
            in_name = batch["filename"][i]
        else:
            in_name = f"sample_{count + 1:04d}"

        in_name = os.path.splitext(in_name)[0]
        out_name = f"{in_name}_t1.png"

        psnr = calc_psnr(pred_01[i:i+1], t2_01[i:i+1])
        ssim = calc_ssim(pred_01[i:i+1], t2_01[i:i+1])

        psnr_total += psnr.item()
        ssim_total += ssim.item()
        count += 1

        save_image(pred_01[i], os.path.join(save_dir, out_name))

        if count % 10 == 0 or count == 1293:
            print(f"[{count}/1293] {out_name} | PSNR {psnr:.2f} | SSIM {ssim:.4f}")
